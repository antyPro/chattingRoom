...
public class WebSocketServer extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
    	String msg ="";
    	if(message.getPayload().startsWith("_$file:")) {
    		msg = "$_file:"+"【" + connections.get(session) + "】" + getDatetime(new Date())+ message.getPayload();
    		
    	}else {
    	
         msg = "【" + connections.get(session) + "】" + getDatetime(new Date()) + " : " + message.getPayload();
    	}
        broadcast(new TextMessage(msg));
    }

    private static void broadcastFile(BinaryMessage msg) {
        // 广播形式发送消息
